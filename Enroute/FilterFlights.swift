...
struct FilterFlights: View {
    @FetchRequest(fetchRequest: Airport.fetchRequst(.all)) var airports: FetchedResults<Airport>
    @FetchRequest(fetchRequest: Airline.fetchRequst(.all)) var airlines: FetchedResults<Airline>
    ...
    var done: some View {
        Button("Done") {
            if self.draft.destination != self.draft.destination {
